...
function printTable(data) {
  ...
  console.log(pad("UNIT", 6) + pad("WEEK", 9) + pad("LAST REP", 13) + pad("ACT/PLAN", 20) + pad("ΔTHIS", 8) + pad("Δ-1W", 8) + pad("Δ-2W", 8) + pad("AVG3", 8) + pad("REQ", 8) + pad("ΔvsREQ", 9) + pad("TARGET", 22) + pad("PROJECTED", 22) + "STATUS");
  ...
  for (const r of rows) {
    ...
    const req = r.reqPace == null ? "past" : r.reqPace.toFixed(1);
    console.log(pad(r.unit, 6) + pad(r.week, 9) + pad(r.lastReported, 13) + pad(ap, 20) + pad(sgn(r.d0), 8) + pad(sgn(r.d1), 8) + pad(sgn(r.d2), 8) + pad(sgn(r.avg3), 8) + pad(req, 8) + pad(sgn(r.paceGap), 9) + pad(target, 22) + pad(proj, 22) + statusOf(r, staleDays));
  }
...
function renderHtml(data) {
  ...
  const body = rows.map((r) => {
    ...
    if (!r.ok) return `<tr class="row-down"><td class="u">${esc(r.unit)}</td><td colspan="13" class="down-note">${esc(r.note || "unreachable")}</td><td><span class="badge unreachable">Unreachable</span></td></tr>`;
    ...
    const gapCls = r.paceGap == null ? "dim" : r.paceGap >= 0 ? "pos" : "neg";
    ...
    return `<tr class="row-${st}"><td class="u">${esc(r.unit)}</td><td class="mono dim">${r.week}</td><td class="mono">${r.lastReported}</td><td class="mono">${r.actual.toFixed(1)}</td><td class="mono dim">${r.planned.toFixed(1)}</td><td class="mono ${r.deviation < 0 ? "neg" : "pos"}">${sgn(r.deviation)}</td>${dcell(r.d0)}${dcell(r.d1)}${dcell(r.d2)}<td class="mono ${r.avg3 == null ? "dim" : r.avg3 >= 0 ? "pos" : "neg"}">${sgn(r.avg3)}</td><td class="mono ${reqCls}">${reqTxt}</td><td class="mono ${gapCls}">${sgn(r.paceGap)}</td><td>${r.target}${tSub ? ` <span class="sub">${tSub}</span>` : ""}</td><td>${pMain}${pSub ? ` <span class="sub">${pSub}</span>` : ""}</td><td><span class="badge ${st}">${BADGE[st]}</span></td></tr>`;
  }).join("\n");
...
.tbl-wrap{overflow-x:auto;border:1px solid var(--line);border-radius:12px;background:var(--card);}table{border-collapse:collapse;width:100%;min-width:1180px;font-size:13.5px;}
...
<div class="tbl-wrap"><table><thead><tr><th class="l">Unit</th><th>Week</th><th>Last reported</th><th>Actual %</th><th>Planned %</th><th>Behind (pp)</th><th>Δ this wk</th><th>Δ −1w</th><th>Δ −2w</th><th>Avg Δ 3wk</th><th>Req Δ/wk</th><th>Δ vs req</th><th class="l">Target delivery</th><th class="l">Projected (recent pace)</th><th class="l">Status</th></tr></thead><tbody>
${body}
